...
function test3DCube() {
	// Test 3x3x3 cube (rectangular prism)
	let strip = neopixel_3d.create(DigitalPin.P0, 27, NeoPixelMode.RGB); // 27 LEDs for 3x3x3
	strip.setCubeWidth(3, 3, 3); // 3x3x3 = 27 LEDs

	// Test setting individual prism positions
	...
	// Test filling prism layer by layer
	for (let z = 0; z < 3; z++) {
		for (let y = 0; y < 3; y++) {
			for (let x = 0; x < 3; x++) {
	...
	// Test 2x3x2 rectangular prism (12 LEDs)
	let prismStrip = neopixel_3d.create(DigitalPin.P2, 12, NeoPixelMode.RGB);
	prismStrip.setCubeWidth(2, 3, 2); // 2x3x2 = 12 LEDs
	// Fill the prism with a gradient
	for (let z = 0; z < 2; z++) {
		for (let y = 0; y < 3; y++) {
			for (let x = 0; x < 2; x++) {
				prismStrip.setCubeColor(
					x,
					y,
					z,
					neopixel_3d.rgb(x * 127, y * 85, z * 127)
				);
			}
		}
	}
	...
	// Test prism with too many LEDs (should set dimensions to 0)
	let bigStrip = neopixel_3d.create(DigitalPin.P1, 20, NeoPixelMode.RGB);
	bigStrip.setCubeWidth(3, 3, 3); // 3x3x3 = 27 > 20, so should become 0
